...
def order_line_rows(
    ...
    for index, order_id in enumerate(order_id_list):
        ...
        for line_number, product_id in enumerate(line_products, start=1):
            ...
            # Distinct prices on the forced double-product demo order
...
def main() -> None:
    ...
    # Expose wave to Hop via properties (Set Variables action) and .period-label.
    # load-e2e-sources-to-crm.hpl uses ${RETAIL_CSV_WAVE} in CSV filenames — do not rewrite the pipeline.
    (files_dir / PERIOD_LABEL_FILE).write_text(wave + "\n", encoding="utf-8")
