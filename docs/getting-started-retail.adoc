...
. Build or download plugin **0.0.16-SNAPSHOT** for Apache Hop **2.18.1** (see link:../README.md[repository README]).
...
|`catalog-data/hop/retail-example/sources/`
|`DV_SOURCE` record definitions (namespace `hop/retail-example/sources`)
...
|`execution-maps/update-retail-dv-bv-dm.hem`
|Optional execution map of the main update graph

|`execution-maps/simulate-6-months.hem`
|Optional execution map of the full six-month simulation workflow
|===
...
Sources are JSON under `catalog-data/`; the namespace in each file must match its folder path (`hop/retail-example/sources`).
...
The workflow drops and recreates schemas, generates CSV source files, loads CRM, validates resource definitions, **measures source data quality and applies a quality gate** (blocking findings stop the load), then runs Data Vault Update, Business Vault Update, and Dimensional Update. After the update it re-measures sources in **alert-only** mode. See link:data-quality.adoc[Data quality].
...
. Open `execution-maps/update-retail-dv-bv-dm.hem` or `execution-maps/simulate-6-months.hem`.
...
|Restart Hop GUI after installing **0.0.16-SNAPSHOT**
