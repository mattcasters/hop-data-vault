...
* **Error** — type mismatch; source string/binary length or numeric precision exceeds the target definition; target database is not Unicode-capable for EDW string storage (SQL Server without a `_UTF8` collation, PostgreSQL non-UTF8 encoding, MySQL/SingleStore without `utf8mb4`)
* **Warning** — source length smaller than target (informational); stored source metadata differs from the live schema (possible DDL drift); SQL Server physical type pairs that can sort differently in hub merge pipelines (for example source `NVARCHAR` vs target `VARCHAR`) — generated SQL auto-applies a bridge `COLLATE` on source and target ORDER BY when live metadata is available; vault VARCHAR uses a UTF-8 collation for storage and expands character lengths to UTF-8 byte lengths (×3) so multi-byte data from `NVARCHAR` sources is not truncated
