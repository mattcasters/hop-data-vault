...
* **Drag** Hubs, Links, and Satellites to position them on the canvas.
* **Click** on any table to edit or delete it.
* **Drag relationships** from one table to another using the middle mouse button or Shift+left-click on a table, then drop on a valid target. Supported pairs are hub↔satellite, hub↔link, and link↔satellite. The model stores the relationship by updating the appropriate name references on the table objects.
* **Click** the background or a table for context actions (add, edit, delete, check, debug, and more).
* **Lasso-select** multiple objects; use the selection toolbar buttons or keyboard shortcuts to copy, cut, paste, or delete.
* **Annotation notes** document the model without affecting loads. Right-click the canvas to add a note, double-click to edit, and drag the resize handles to change size. Notes support types (General, Important, Warning, Information) and lightweight link syntax: `[Label](https://example.org)` opens a URL; `[Label](TableName)` navigates to a table in the model.
...
|Check model
|Runs validation and shows results in a Hop check dialog (missing keys, broken references, duplicate names, and more).
...
**Check model** runs the same validations used by the Data Vault Update action. It warns about missing business keys, unattached satellites, links with fewer than two hubs, duplicate names, references to sources or hubs that do not exist, and many other common problems.

**Debug** generates the actual update pipeline(s) for the selected table(s) and opens them in Hop so you can review or run them manually. When the model defines a generated-pipeline folder, debug pipelines can be saved there as well.
...
* Can be configured to log issues or abort if errors are found.
