...
* Record definitions published to that catalog (for example Data Vault sources under namespace `hop/project/sources`).
...
|Namespace field
|Enabled when **Select from previous transform input?** is on. Name of the incoming field that holds the catalog namespace (for example `hop/project/sources`).
...
`project/tests/shared/create-source-tables-from-data-catalog.hpl` runs at the start of `project/tests/run-tests.hwf`:
...
2. **Filter rows** — keeps definitions in namespace `hop/project/sources` whose name starts with `CRM`.
...
`project/tests/shared/create-source-table-from-data-catalog.hpl` creates one source table from a single catalog entry:
