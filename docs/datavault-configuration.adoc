...
On **SQL Server**, generated hub (and satellite target) SQL automatically inspects live source/target column collations and types. When a sort risk is detected (for example `NVARCHAR` vs `VARCHAR`, or different collations), the same bridge `COLLATE` is applied on both source and target `ORDER BY` expressions so Merge Rows streams stay aligned without manual configuration. Vault `VARCHAR` columns are created with the UTF-8 collation `Latin1_General_100_CI_AS_SC_UTF8` (SQL Server 2019+) so Unicode values store without requiring `NVARCHAR`. Because SQL Server measures UTF-8 `VARCHAR(n)` in *bytes* while source/`NVARCHAR` lengths are character-oriented, generated DDL multiplies string lengths by three (and promotes to `VARCHAR(MAX)` when that exceeds 8000) so multi-byte values such as accented text cannot truncate on insert.

=== Unicode capability (EDW requirement)

Data Vault, Business Vault, and dimensional models require a **Unicode-capable target database**. Model check (and update actions that enforce model check) refuse to treat non-Unicode targets as valid EDW destinations:
...
* **MySQL / MariaDB / SingleStore** — schema character set must be `utf8mb4` (plain `utf8` or `latin1` is rejected).

If the vault database is unreachable during check, a **warning** is raised so offline design can continue; when connectivity is available and the encoding is wrong, an **error** blocks the update.
