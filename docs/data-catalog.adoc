...
Use tagged versions as the expected contract for the **Validate resource definitions** workflow action (compare tagged catalog vs live source or vs working tree, with optional Markdown/HTML reports). DTAP promotion steps are in link:resource-definition-validation.adoc#dtap-promotion-recipe[Resource definition validation — DTAP recipe].
