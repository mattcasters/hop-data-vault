...
public final class SortedSchemaMergeLogic {
  ...
  public static int compareRows(
    ...
    int[][] leftIndices = sortKeyIndices;
    int[][] rightIndices = sortKeyIndices;
    for (int keyIndex = 0; keyIndex < sortKeys.size(); keyIndex++) {
      SortedSchemaMergeSortKey sortKey = sortKeys.get(keyIndex);
      int leftFieldIndex = leftIndices[left.getStreamIndex()][keyIndex];
      int rightFieldIndex = rightIndices[right.getStreamIndex()][keyIndex];
