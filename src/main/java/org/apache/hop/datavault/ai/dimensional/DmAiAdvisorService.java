...
/** Calls the configured LLM and parses dimensional model advisory responses. */
public final class DmAiAdvisorService {

  private DmAiAdvisorService() {}

  public static DvAiResponse advise(
      HopAiConfig config, IVariables variables, DmAiContextBundle context, List<ChatMessage> history)
      throws HopException {
  ...
  static String buildSystemPrompt(DmAiContextBundle context) throws HopException {
  ...
  private static String buildUserPromptBody(DmAiContextBundle context, boolean includeFullContext) {
    ...
    prompt.append("Dimensional model structure JSON:\n")
        .append(nullToEmpty(context.getModelStructureJson()))
        .append("\n\n");

    if (includeFullContext) {
      prompt.append("Dimensional model summary JSON:\n")
