...
@Getter
@Setter
public abstract class BasePainter<P extends IBaseMeta> {
    ...
    protected void drawOriginBoundary() {
        ...
        if (ox > 0 && oy > 0) {
            drawHatching(0, ox, (int) visH); // left strip (full height)
            int topW = (int) visW - ox;
            if (topW > 0) {
                drawHatching(ox, topW, oy); // top strip (no overlap)
            }
        } else if (ox > 0 && ox < visW) {
            drawHatching(0, ox, (int) visH);
        } else if (oy > 0 && oy < visH) {
            drawHatching(0, (int) visW, oy);
        }
     ...
     */
    private void drawHatching(int x, int w, int h) {
        if (w <= 0 || h <= 0) {
            return;
        }
        gc.setForeground(EColor.DARKGRAY);
